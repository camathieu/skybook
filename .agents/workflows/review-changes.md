...
- [ ] Do changes use Tailwind design tokens instead of hardcoded colors?
- [ ] Do new/modified components look correct in the dark theme?
...
- [ ] Does each language follow its own punctuation rules?
...
If VitePress docs were changed, validate the docs build:

```bash
make docs 2>&1 | tail -20
```

### 5b. Vulnerability check (if available)
